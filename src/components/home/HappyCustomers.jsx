...
const reviews = [
    ...
    },
    {
        ...
        city: "Islamabad",
        rating: 5,
        text: "Zeccora feels like a real luxury brand. I will definitely order another piece.",
    },
];

const HappyCustomers = () => {
    return (
        <section className="bg-[#f7f2ec] py-16 text-[#1a120c] sm:py-20">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="mb-12 text-center">
                    ...
                    <h2 className="display-font text-3xl sm:text-5xl lg:text-6xl">
                        What our <span className="italic text-[#9F6324]">customers</span> say
                    </h2>
                    <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[#6b5b4e] sm:text-base">
                        Notes from shoppers across Pakistan after they received their Zeccora bags.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
                    {reviews.map((review) => (
                        <article
                            key={review.name}
                            className="flex h-full flex-col rounded-[28px] bg-white p-6 text-[#1a120c] shadow-[0_10px_30px_rgba(26,18,12,0.05)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(26,18,12,0.1)]"
                        >
                            <div className="flex items-center justify-between">
                                ...
                                <Quote size={18} className="text-[#9F6324]" />
                            </div>
                            <div className="mt-4 flex gap-1">
                                {Array.from({ length: 5 }).map((_, index) => (
                                    <Star key={index} size={13} className={index < review.rating ? "fill-[#9F6324] text-[#9F6324]" : "text-[#d7c9bb]"} />
                                ))}
                            </div>
                            <p className="mt-4 flex-1 text-sm leading-6 text-[#5c4c40]">{review.text}</p>
                            <div className="mt-6 border-t border-black/5 pt-4">
                                <p className="text-sm font-semibold">{review.name}</p>
                                <p className="mt-1 text-xs uppercase tracking-[0.16em] text-[#9F6324]">{review.city} · Verified order</p>
                            </div>
                        </article>
                    ))}
                </div>
